...
const indexEventShows = async () => {
  ...
  try {
    ...
    if (SEND_TO_ALGOLIA) {
      console.log('🧹 Pulizia record "event" esistenti (rimuove duplicati/orfani prima del reindex)...');
      await algoliaService.deleteByFilters(ALGOLIA_INDEX_NAME, 'type:event');
