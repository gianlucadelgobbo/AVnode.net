...
passport.flxerLogin((existingUser, email, password, done) => {
  // try with flxer api
  request.post({
  ...
  User.findOne({ email: email.toLowerCase() }, (err, user) => {
    ...
    user.comparePassword(password, (err, isMatch) => {
      ...
      if (isMatch) {
        ...
        passport.flxerLogin(user, email, password, (isFlxerMatch) => {
          if (isFlxerMatch) {
            logger.debug('passport.use User.comparePassword flxer match');
            return done(null, user);
          } else {
