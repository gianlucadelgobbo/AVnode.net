...
passport.use(new LocalStrategy({ usernameField: 'email' }, (email, password, done) => {
  ...
  User.findOne({ email: email.toLowerCase() }, (err, user) => {
    ...
    if (!user) {
      ...
      logger.debug('passport.use User.findOne user found:' + email);
    }
    if (!user.password) {
    ...
    user.comparePassword(password, (cErr, isMatch) => {
      // logger.debug('passport.use password:' + password);
      if (cErr) {
        logger.debug('passport.use User.comparePassword error:' + JSON.stringify(cErr));
        return done(cErr); 
      }
      if (isMatch) {
