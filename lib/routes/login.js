...
router.post('/', (req, res, next) => {
  ...
  fileLogger.info('passport.authenticate req:' + JSON.stringify(req.body.email));
  ...
  passport.authenticate('local', (err, user, info) => {
    ...
    if (!user) {
      logger.debug('passport.authenticate !user:' + JSON.stringify(info) + ', trying on flxer with email:' + JSON.stringify(req.body.email));
      fileLogger.error('passport.authenticate !user:' + JSON.stringify(info) + ', trying on flxer with email:' + JSON.stringify(req.body.email));
      // try with flxer api
      request.post({
        uri: 'https://flxer.net/api/login',
        headers: { 'content-type': 'application/x-www-form-urlencoded' },
        body: querystring.stringify({ email: req.body.email, password: req.body.password })
      }, function (fErr, resp, body) {
        if (fErr) {
          logger.error('flxer.authenticate error:' + JSON.stringify(fErr));
          fileLogger.error('errors', 'flxer.authenticate error:' + JSON.stringify(fErr));
        }
        ...
        if (ress.login) {
          ...
          fileLogger.info('errors', 'flxer.authenticate successful');
          User.findOne({ email: req.body.email }, (uErr, existingUser) => {
            if (uErr) {
              logger.error('User.findOne error:' + uErr);
              fileLogger.error('errors', 'User.findOne error:' + uErr);
            }
            if (existingUser) {
              ...
              existingUser.save((sErr) => {
                if (sErr) {
                  ...
                  fileLogger.error('errors', 'existingUser.save error:' + sErr);
                }
                ...
                /* passport.authenticate('local', (err, existingUser, info) => {
                  req.logIn(existingUser, (loginErr) => {
                    if (loginErr) {
                      ...
                      fileLogger.error('passport flxer.logIn error:' + JSON.stringify(loginErr));
                      return next(loginErr);
                    }
                    logger.info('passport flxer auth success');
                    req.flash('success', { msg: i18n.__('You are logged in, flxer account migrated.') });
                    fileLogger.info('success', { msg: i18n.__('You are logged in, flxer account migrated.') });
                    return res.redirect('/');
                  });
                });  */
                req.logIn(existingUser, (loginErr) => {
                  if (loginErr) {
                    logger.error('passport flxer.logIn2 error:' + JSON.stringify(loginErr));
                    fileLogger.error('passport flxer.logIn2 error:' + JSON.stringify(loginErr));
                    return next(loginErr);
                  }
                  logger.info('passport flxer2 auth success');
                  req.flash('success', { msg: i18n.__('You are logged in, flxer.net account migrated.') });
                  fileLogger.info('success', { msg: i18n.__('You are logged in, flxer.net account migrated.') });
                  return res.redirect('/');
                });           
                // BL CHECK end
                mailer.sendMsgEmail({ to: existingUser.email }, { msg: 'Password migrated from flxer, please login again.' }, (mErr) => {
                  if (mErr) {
                    ...
                    fileLogger.error('errors', 'mailer.sendMsgEmail error:' + mErr);
                  }
                  req.flash('success', { msg: i18n.__('Please check your inbox for information.') });
                  fileLogger.info('success', { msg: i18n.__('Please check your inbox for information.') });
                });
              });
            } else {
              logger.error('user with this email not found in db');
              fileLogger.error('errors', 'user with this email not found in db');
            }
          });

        } else {
          logger.error('flxer.authenticate failed');
          fileLogger.error('errors', 'flxer.authenticate failed');
        }
      });
