  ...
  passport.authenticate('local', (err, user, info) => {
    ...
    if (!user) {
      ...
      request.post({
        ...
        if (fErr) {
          logger.debug('flxer.authenticate error:' + JSON.stringify(fErr));
        ...
        if (ress.login) {
          ...
          User.findOne({ email: req.body.email }, (uErr, existingUser) => {
            if (uErr) {
              logger.debug('User.findOne error:' + uErr);
            ...
            if (existingUser) {
              ...
              existingUser.save((sErr) => {
                if (sErr) {
                  logger.debug('existingUser.save error:' + sErr);
                ...
                mailer.sendMsgEmail({ to: existingUser.email }, { msg: 'Password migrated from flxer, please login again.' }, (mErr) => {
                  if (mErr) {
                    logger.debug('mailer.sendMsgEmail error:' + mErr);
