...
const fetchUserCrewMembers = (id, cb) => {
  User
    .findById(id)
    .populate({
      path: 'crews',
      ...
      if (crew) {
        if (crew.members) {
          ...
          if (!memberFound) {
            ...
            fetchUserCrewMembers(req.params.memberId, (mErr, member) => {
              if (mErr) {
                logger.debug(`${apiCall} fetchUserCrewMembers ERROR: ${JSON.stringify(mErr)}`);
              } else {
                if (member && member.email) {
                  logger.info(`${apiCall} fetchUserCrewMembers member: ${JSON.stringify(member)}`);
                  ...
                  logger.info(`${apiCall} fetchUserCrewMembers email: ${member.email}`);
                  ...
                  if (member) {
                    logger.debug(`${apiCall} fetchUserCrewMembers member email undefined: ${JSON.stringify(member)}`);
                  } else {
                    logger.debug(`${apiCall} fetchUserCrewMembers member undefined, reindex elasticsearch?`);
                  }
                }
              }
            });
