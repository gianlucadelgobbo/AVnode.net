...
const fetchUserBasicInfo = (id, cb) => {
  User
    .findById(id)
    .exec(cb);
};
    ...
    .exec((err, crew) => {
      ...
      if (crew) {
        if (crew.members) {
          ...
          crew.members.map((m) => {
            logger.debug(`${apiCall} compare member ${m} to id ${req.params.memberId} `);
            // BL notice the == instead of ===
            if (m == req.params.memberId) {
              // member is found
              logger.debug(`${apiCall} member found for this crew, not adding`);
              memberFound = true;
            }
          });
          // in case of new member, add it to the members
          if (!memberFound) {
            // BL try to get members email
            fetchUserBasicInfo(req.params.memberId, (mErr, member) => {
              if (mErr) {
                logger.debug(`${apiCall} fetchUserBasicInfo ERROR: ${JSON.stringify(mErr)}`);
              } else {
                if (member && member.email) {
                  logger.debug(`${apiCall} fetchUserBasicInfo email: ${member.email}`);
                  // email found, we can add the member
                  crew.members.push(req.params.memberId);
                  // send email to member
                  // BL TODO get members email, verify email confirmed, add crew to members account
                  ...
                  if (member) {
                    logger.debug(`${apiCall} fetchUserBasicInfo member email undefined: ${JSON.stringify(member)}`);
                  } else {
                    logger.debug(`${apiCall} fetchUserBasicInfo member undefined`);
                  }
                }
              }
            });        
