      ...
      if (organizingcrew) {
        if (organizingcrew.events) {
          ...
          organizingcrew.events.map((p) => {
            ...
            if (p == req.params.id) {
              // event is found
              logger.debug(`api, router.post(/event/:id/organizingcrew/:organizingcrewId) organizingcrew found for this crew, not adding`);
              eventFound = true;
            }
          });
        ...
        organizingcrew.save((_err) => {
          if (_err) {
            logger.debug(`api, router.post(/event/:id/organizingcrew/:organizingcrewId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
      ...
      if (user) {
        if (user.crews) {
          ...
          user.crews.map((c) => {
            ...
            if (c == req.params.id) {
              // crew is found
              logger.debug(`api, router.post(/crew/:id/member/:memberId) crew found for this user, not adding`);
              crewFound = true;
            }
          });
        ...
        user.save((_err) => {
          if (_err) {
            logger.debug(`api, router.post(/crew/:id/member/:memberId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
      ...
      if (crew) {
        if (crew.performances) {
          ...
          crew.performances.map((c) => {
            ...
            if (c == req.params.id) {
              // perf is found
              logger.debug(`api, router.post(/performance/:id/crew/:crewId) perf found for this crew, not adding`);
        ...
        crew.save((_err) => {
          if (_err) {
            logger.debug(`api, router.post(/performance/:id/crew/:crewId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
  ...
  User
    ...
    .exec((err, performer) => {
      ...
      if (performer) {
        if (performer.performances) {
          ...
          performer.performances.map((p) => {
            ...
            if (p == req.params.id) {
              ...
              logger.debug(`api, router.post(/performance/:id/performer/:performerId) performances found for this performer, not adding`);
              performanceFound = true;
            }
          });
          // in case of new performance, add it to the performances
        ...
        performer.save((_err) => {
          if (_err) {
            logger.debug(`api, router.post(/performance/:id/performer/:performerId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
