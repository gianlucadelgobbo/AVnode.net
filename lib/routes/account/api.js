  ...
  User.findById(req.params.id, (err, user) => {
    ...
    if (user) {
      if (user.emails) {
        ...
        if (!user.emails[req.params.emailId].is_confirmed) {
          user.emails[req.params.emailId].confirm = uuid.v4();
          logger.debug(`${apiCall} email ${user.emails[req.params.emailId].email}`);
          mailer.confirmEmail({ to: user.emails[req.params.emailId].email }, { uuid: user.emails[req.params.emailId].confirm }, (mErr) => {
            if (mErr) {
              logger.debug(`${apiCall} mailer ERROR ${JSON.stringify(mErr)}`);
            }
