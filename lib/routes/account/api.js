...
router.post('/event/venue', (req, res) => {
  // BL TODO venue is undefined if not the first venue in the event
  console.log('router.post(/event/venue)');
  console.log('SAVING /event/venue, req.body:' + JSON.stringify(req.body));
  const newVenue = new Venue({
    slug: slugify(req.body.location.name),
    name: req.body.location.name, // BL friendly name indexed for search
    address: req.body.location
  }).save((err, venue) => {
    console.log('SAVED /event/venue, req.body.id' + req.body.id);
    Event
      .findById(req.body.id, (err, event) => {
        console.log('/event/venue, event' + JSON.stringify(event) );
        
        if (venue) {
          console.log('OK /event/venue, venue found:' + venue.name);
          event.venues.push(venue._id);
        } else {
          console.log('ERROR /event/venue: venue is undefined')
        }
        event.save((err) => {
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
