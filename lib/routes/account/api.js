...
router.put('/event/:id/performance/:performanceId', (req, res) => {
  Performance
    ...
    .exec((err, performance) => {
      if (err) {
        logger.debug(`api, router.put(/event/:id/performance/:performanceId) findById ERROR: ${JSON.stringify(err)}`);
      }
      if (performance) {
        if (performance.events) {
          ...
          performance.events.map((e) => {
            ...
            if (e == req.params.id) {
              // event is found
              logger.debug(`api, router.put(/event/:id/performance/:performanceId) event found for this performance, not adding`);
              eventFound = true;
            }
          });
        ...
        performance.save((_err) => {
          if (_err) {
            logger.debug(`api, router.put(/event/:id/performance/:performanceId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
        ...
        logger.debug(`api, router.put(/event/:id/performance/:performanceId) ERROR performance is null, reindex elasticsearch?`);
      }
...
router.delete('/event/:id/performance/:performanceId', (req, res) => {
  Performance
    ...
    .exec((err, performance) => {
      ...
      performance.save((_err) => {
        if (_err) {
          logger.debug(`api, router.delete(/event/:id/performance/:performanceId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.put('/event/:id/organizer/:organizerId', (req, res) => {
  User
    ...
    .exec((err, organizer) => {
      if (err) {
        logger.debug(`api, router.put(/event/:id/organizer/:organizerId) findById ERROR: ${JSON.stringify(err)}`);
      }
      if (organizer) {
        if (organizer.events) {
          let eventFound = false;
          organizer.events.map((c) => {
            ...
            if (c == req.params.id) {
              // event is found
              logger.debug(`api, router.put(/event/:id/organizer/:organizerId) event found for this organizer, not adding`);
              eventFound = true;
            }
          });
          // in case of new event, add it to the events
        ...
        organizer.save((_err) => {
          if (_err) {
            logger.debug(`api, router.put(/event/:id/organizer/:organizerId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
        ...
        logger.debug(`api, router.put(/event/:id/organizer/:organizerId) ERROR organizer is null, reindex elasticsearch?`);
      }
    });
});
...
router.delete('/event/:id/organizer/:organizerId', (req, res) => {
  User
    ...
    .exec((err, organizer) => {
      ...
      organizer.save((_err) => {
        if (_err) {
          logger.debug(`api, router.delete(/event/:id/organizer/:organizerId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.put('/event/:id/organizingcrew/:organizingcrewId', (req, res) => {
  Crew
    ...
    .exec((err, organizingcrew) => {
      if (err) {
        logger.debug(`api, router.put(/event/:id/organizingcrew/:organizingcrewId) findById ERROR: ${JSON.stringify(err)}`);
      }
      if (organizingcrew) {
        if (organizingcrew.events) {
          ...
          organizingcrew.events.map((p) => {
            ...
            if (p == req.params.id) {
              // event is found
              logger.debug(`api, router.put(/event/:id/organizingcrew/:organizingcrewId) organizingcrew found for this crew, not adding`);
              eventFound = true;
            }
          });
          // in case of new event, add it to the events
        ...
        organizingcrew.save((_err) => {
          if (_err) {
            logger.debug(`api, router.put(/event/:id/organizingcrew/:organizingcrewId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
        ...
        logger.debug(`api, router.put(/event/:id/organizingcrew/:organizingcrewId) ERROR organizingcrew is null, reindex elasticsearch?`);
      }
    });
});
...
router.delete('/event/:id/organizingcrew/:organizingcrewId', (req, res) => {
  Crew
    ...
    .exec((err, organizingcrew) => {
      ...
      organizingcrew.save((_err) => {
        if (_err) {
          logger.debug(`api, router.delete(/event/:id/organizingcrew/:organizingcrewId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.delete('/event/:id/category/:categoryId', (req, res) => {
  logger.debug(`api, router.delete(/event/:id/category/:categoryId) delete category: ${req.params.categoryId} from event ${req.params.id} user ${req.user.id}`);
  Event
    ...
    .exec((err, event) => {
      ...
      event.save((_err) => {
        if (_err) {
          logger.debug(`api, router.delete(/event/:id/category/:categoryId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.post('/event/venue', (req, res) => {
  logger.info(`api, router.post('/event/venue') ${JSON.stringify(req.body)}`);
  const newVenue = new Venue({
    slug: slugify(req.body.location.name),
    // BL now in the address: name: req.body.location.name, // BL friendly name indexed for search
    address: req.body.location,
    place_id: req.body.location.place_id
  }).save((err, venue) => {
    if (err) {
      logger.debug(`api, router.post(/event/venue) ERROR:' ${JSON.stringify(err)}`);
    }
    Event
      .findById(req.body.id, (err, event) => {
        if (err) {
          logger.debug(`api, router.post(/event/venue) Event.findById ERROR:' ${JSON.stringify(err)}`);
        }
        if (venue) {
          logger.info(`api, router.post('/event/venue') OK venue found: ${venue.name}`);
          event.venues.push(venue._id);
        } else {
          logger.debug('api, router.post(/event/venue) ERROR venue is undefined');
        }
        event.save((err) => {
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
...
router.delete('/event/:id/venue/:venueId', (req, res) => {
  Event
    ...
    .exec((err, event) => {
      ...
      event.save((_err) => {
        if (_err) {
          logger.debug(`api, router.delete(/event/:id/venue/:venueId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
...
formatAddress = (location, name, primary) => {
  let street_number = '';
  let route = '';
  let postal_code = '';
  let locality = '';
...
router.post('/user/place', (req, res, next) => {
  ...
  User
    .findById(req.body.id, (err, user) => {
      ...
      user.addresses.push(formatAddress(req.body.location, req.body.location.name, primary));// was req.body.location.address);
      user.save((err) => {
        if (err) {
          return next(err);
        }
...
router.post('/crew', (req, res) => {
  ...
  console.log('crew' + JSON.stringify(req.body));
  const newCrew = new Crew(Object.assign({}, req.body, {
    name: req.body.title,
    slug: newSlug,
    members: []
  }));
  console.log('newcrew' + JSON.stringify(newCrew));

  newCrew.save((saveerr, crew) => {
    if (saveerr) {
      logger.debug(`api, router.post(/crew) save ERROR: ${JSON.stringify(saveerr)}`);
    }

    fetchUser(req.user.id, (err, user) => {
      if (err) {
        logger.debug(`api, router.post(/crew) fetchUser ERROR: ${JSON.stringify(err)}`);
        throw err;
      }
      if (saveerr) {
        res.json(user);
...
router.put('/crew/:id/member/:memberId', (req, res) => {
  User
    ...
    .exec((err, user) => {
      if (err) {
        logger.debug(`api, router.put(/crew/:id/member/:memberId) findById ERROR: ${JSON.stringify(err)}`);
      }
      ...
      if (user) {
        if (user.crews) {
          ...
          user.crews.map((c) => {
            ...
            if (c == req.params.id) {
              // crew is found
              logger.debug(`api, router.put(/crew/:id/member/:memberId) crew found for this user, not adding`);
              crewFound = true;
            }
          });
        ...
        user.save((_err) => {
          if (_err) {
            logger.debug(`api, router.put(/crew/:id/member/:memberId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
        ...
        logger.debug(`api, router.put(/crew/:id/member/:memberId) ERROR user is null, reindex elasticsearch?`);
      }
    });
});
...
router.delete('/crew/:id/member/:memberId', (req, res) => {
  User
    ...
    .populate({
      ...
      user.save((_err) => {
        if (_err) {
          logger.debug(`api, router.delete(/crew/:id/member/:memberId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.put('/crew/:id', (req, res) => {
  ...
  Crew
    .findById(req.params.id, (err, crew) => {
      if (err) {
        logger.debug(`api, router.put(/crew/:id) findById ERROR: ${JSON.stringify(err)}`);
      }
      if (crew) {
        ...
        logger.debug(`api, router.put(/crew/:id) ERROR crew is null`);
      }
    });
});
...
router.delete('/performance/:id/category/:categoryId', (req, res) => {
  logger.debug(`api, router.delete(/performance/:id/category/:categoryId) delete category: ${req.params.categoryId} from performance ${req.params.id} user ${req.user.id}`);
  Performance
    .findById(req.params.id)
    .exec((err, performance) => {
      if (err) {
        logger.debug(`api, router.delete(/performance/:id/category/:categoryId) findById ERROR: ${JSON.stringify(_err)}`);
      }
      performance.categories.remove(req.params.categoryId);
      performance.save((_err) => {
        if (err) {
          logger.debug(`api, router.delete(/performance/:id/category/:categoryId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.post('/performance/:id/video', (req, res) => {
  ...
  console.log('api, router.put(/performance/:id/video) video:', req.body.video);
  assetUtil.createVideoAsset(req.body.video, (err, assetId) => {
    if (err) {
      logger.debug(`api, router.put(/performance/:id/video) assetUtil.createVideoAsset ERROR: ${JSON.stringify(err)}`);
    }

    Performance.findById(req.params.id, (err, performance) => {
      if (err) {
        logger.debug(`api, router.put(/performance/:id/video) Performance.findById ERROR: ${JSON.stringify(err)}`);
      }
      if (performance.video) {
        performance.video = assetId;
      } else {
        logger.debug(`api, router.put(/performance/:id/video) performance.video is null`);
      }
      performance.save(() => {
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
...
router.put('/performance/:id/crew/:crewId', (req, res) => {
  Crew
    ...
    .exec((err, crew) => {
      if (err) {
        logger.debug(`api, router.put(/performance/:id/crew/:crewId) findById ERROR: ${JSON.stringify(err)}`);
      }
      if (crew) {
        if (crew.performances) {
          ...
          crew.performances.map((c) => {
            ...
            if (c == req.params.id) {
              // perf is found
              logger.debug(`api, router.put(/performance/:id/crew/:crewId) perf found for this crew, not adding`);
        ...
        crew.save((_err) => {
          if (_err) {
            logger.debug(`api, router.put(/performance/:id/crew/:crewId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
        ...
        logger.debug(`api, router.put(/performance/:id/crew/:crewId) ERROR crew is null, reindex elasticsearch?`);
      }
    });
});
...
router.delete('/performance/:id/crew/:crewId', (req, res) => {
  Crew
    ...
    .exec((err, crew) => {
      ...
      crew.save((_err) => {
        if (_err) {
          logger.debug(`api, router.delete(/performance/:id/crew/:crewId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.put('/performance/:id/performer/:performerId', (req, res) => {
  User
    ...
    .exec((err, performer) => {
      if (err) {
        logger.debug(`api, router.put(/performance/:id/performer/:performerId) findById ERROR: ${JSON.stringify(err)}`);
      }
      if (performer) {
        if (performer.performances) {
          ...
          performer.performances.map((p) => {
            ...
            if (p == req.params.id) {
              ...
              logger.debug(`api, router.put(/performance/:id/performer/:performerId) performances found for this performer, not adding`);
              performanceFound = true;
            }
          });
          // in case of new performance, add it to the performances
        ...
        performer.save((_err) => {
          if (_err) {
            logger.debug(`api, router.put(/performance/:id/performer/:performerId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
          }
          fetchUser(req.user.id, (err, user) => {
            res.json(user);
          });
        ...
        logger.debug(`api, router.put(/performance/:id/performer/:performerId) ERROR performer is null, reindex elasticsearch?`);
      }
    });
});
...
router.delete('/performance/:id/performer/:performerId', (req, res) => {
  User
    ...
    .exec((err, performer) => {
      ...
      performer.save((_err) => {
        if (_err) {
          logger.debug(`api, router.delete(/performance/:id/performer/:performerId) save ERROR: ${JSON.stringify(_err)}  ${req.params.id}`);
        }
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.put('/user/:id', (req, res) => {
  ...
  User.findById(req.user.id, (err, user) => {
    if (err) {
      logger.debug(`api, router.put(/user/:id) findById ERROR: ${JSON.stringify(err)}`);
    }
    if (user) {
      ...
      logger.debug(`api, router.put(/user/:id) ERROR user is null`);
    }
  });
});
