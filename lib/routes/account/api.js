...
router.post('/event/:id/image', up3, (req, res, next) => {
  ...
  const apiCall = `api, router.post(/event/${JSON.stringify(req.params.id)}/image)`;
  async.parallel({
    image: (cb) => {
      if (req.files['image']) {
...
router.post('/event/:id/teaser', up4, (req, res, next) => {
  // FIXME: Why next() as error handling?
  // FIXME: Delete old asset if there is one
  const apiCall = `api, router.post(/event/${JSON.stringify(req.params.id)}/teaser)`;
  async.parallel({
    image: (cb) => {
      if (req.files['image']) {
        const file = req.files['image'][0];
...
router.post('/user/:id/image/profile', up, (req, res, next) => {
  // FIXME: Why next() as error handling?
  // FIXME: Delete old asset if there is one
  const apiCall = `api, router.post(/user/${JSON.stringify(req.user.id)}/image/profile`;
  console.log( JSON.stringify(req.files['image'][0]) + ' req:' + JSON.stringify(req));
  logger.debug(`${apiCall} req: ${JSON.stringify(req)}`);
  logger.debug(`${apiCall} file: ${JSON.stringify(req.files['image'][0])}`);
  async.parallel({
    image: (cb) => {
      if (req.files['image']) {
        const file = req.files['image'][0];
