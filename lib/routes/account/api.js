...
// edit user web link
router.put('/user/:id/linkweb/:linkId', (req, res) => {
  const apiCall = `api, router.put(/user/${JSON.stringify(req.params.id)}/linkweb/${JSON.stringify(req.params.linkId)})`;
  logger.debug(`${apiCall} req.params:' ${JSON.stringify(req.params)}`);
      ...
      if (user.links) {
        ...
        user.links.map((l) => {
          ...
          if (l._id == req.params.linkId) {
            // link is found
            logger.debug(`${apiCall} web link found ${l.url}`);
            user.linkWeb = l.url;
          }
        });
      }
...
// edit user social link
router.put('/user/:id/linksocial/:linkId', (req, res) => {
  const apiCall = `api, router.put(/user/${JSON.stringify(req.params.id)}/linksocial/${JSON.stringify(req.params.linkId)})`;
  logger.debug(`${apiCall} req.params:' ${JSON.stringify(req.params)}`);
    ...
    if (user) {
      if (user.links) {
        ...
        user.links.map((l) => {
          ...
          if (l._id == req.params.linkId) {
            // link is found
            logger.debug(`${apiCall} social link found ${l.url}`);
            user.linkSocial = l.url;
            user.linkSocialType = l.type;
          }
        });
      }
      user.save((err) => {
...
router.put('/user/:id', (req, res) => {
  ...
  if (!linkSocialFound) {
    let newLinkSocial = { type: req.body.linkSocialType, url: req.body.linkSocial };
