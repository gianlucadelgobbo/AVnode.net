    ...
    .exec((err, crew) => {
      ...
      if (crew) {
        ...
        crew.save((_err) => {
          ...
          // BL WIP get members email
          fetchCrewMember(req.params.memberId, (mErr, member) => {
            if (mErr) {
              logger.debug(`${apiCall} fetchCrewMemberEmail ERROR: ${JSON.stringify(mErr)}`);
            }
            logger.debug(`${apiCall} fetchCrewMemberEmail email: ${member.email}`);
