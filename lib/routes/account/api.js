...
// for user address only (placename is set to locality, because it's required)
formatAddress = (location, primary) => {
  let street_number = '';
  let route = '';
  let postal_code = '';
  let locality = '';
  ...
  const address = {
    ...
    placename: locality, // BL friendly name: placename is set to locality, because it's required
    is_primary: primary
  };
  return address;
};
...
router.post('/user/place', (req, res, next) => {
  ...
  User
    .findById(req.body.id, (err, user) => {
      ...
      user.addresses.push(formatAddress(req.body.location, primary));
      user.save((err) => {
        if (err) {
          return next(err);
        }
...
router.delete('/user/:id/about/:aboutId', (req, res) => {
  User
    .findById(req.params.id)
    .exec((err, user) => {
      // BL TODO user.abouts remove req.params.aboutId ;
      user.abouts.remove(req.params.aboutId);
      user.save((_err) => {
        fetchUser(req.user.id, (err, user) => {
          res.json(user);
        });
...
router.put('/user/:id/makeemailpublic/:emailId', (req, res) => {
  const apiCall = `api, router.put(/user/${JSON.stringify(req.params.id)}/makeemailpublic/${JSON.stringify(req.params.linkId)})`;
  logger.debug(`${apiCall} req.params:' ${JSON.stringify(req.params)}`);
...
router.put('/user/:id/emailconfirm/:emailId', (req, res) => {
  const apiCall = `api, router.put(/user/${JSON.stringify(req.params.id)}/emailconfirm/${JSON.stringify(req.params.linkId)})`;
  logger.debug(`${apiCall} req.params:' ${JSON.stringify(req.params)}`);
