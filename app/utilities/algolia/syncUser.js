...
export const syncUserToAlgolia = async (userId) => {
  ...
  // Cascade: performances, events, galleries, videos, crews and members
  try {
    const performanceIds = Array.from(new Set((user.performances || [])
      .map(p => p && p._id && p._id.toString())
      .filter(Boolean)));
    performanceIds.forEach((id) => {
      syncPerformanceToAlgolia(id).catch((e) => logger.error('Algolia cascade sync (performance from user) failed', e));
    });
    ...
    const eventIds = Array.from(new Set((user.events || [])
      ...
      .filter(Boolean)));
    eventIds.forEach((id) => {
      syncEventToAlgolia(id).catch((e) => logger.error('Algolia cascade sync (event from user) failed', e));
    });
    ...
    galleryIds.forEach((id) => {
      syncGalleryToAlgolia(id).catch((e) => logger.error('Algolia cascade sync (gallery from user) failed', e));
    });
    ...
    const videoIds = Array.from(new Set((user.videos || [])
      ...
      .filter(Boolean)));
    videoIds.forEach((id) => {
      syncVideoToAlgolia(id).catch((e) => logger.error('Algolia cascade sync (video from user) failed', e));
    });
    ...
    Array.from(relatedUserIds).forEach((id) => {
      syncUserToAlgolia(id).catch((e) => logger.error('Algolia cascade sync (crew/member user from user) failed', e));
    });
  } catch (e) {
