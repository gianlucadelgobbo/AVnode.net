...
const gallerySchema = new Schema({
  ...
  gallery_original: { type: mongoose.Schema.Types.ObjectId, ref: 'Gallery', required: true },
  createdAt: Date,
  ...
  slug:  { type: String, trim: true, required: [true, 'GALLERY_URL_IS_REQUIRED'], minlength: [3, 'GALLERY_URL_IS_TOO_SHORT'], maxlength: [100, 'GALLERY_URL_IS_TOO_LONG'],
    validate: [(slug) => {
      var re = /^[a-z0-9-_]+$/;
      return re.test(slug)
  ...
  collection: 'event_freezed_galleries'
});
...
const EventFreezedGallery = mongoose.model('EventFreezedGallery', gallerySchema);

export default EventFreezedGallery;
