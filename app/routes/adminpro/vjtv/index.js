...
router.get('/', (req, res) => {
  ...
  if (req.query.month) {
    console.log("eccomi");
    Video.
    find({"categories.0":{$exists:true},"media.duration": {$gt:0}, "media.encoded": 1}).
    sort({createdAt: -1}).
    ...
    //lean().
    exec((err, data) => {
      let date = new Date(req.query.month+"-01");
      let enddate = new Date(new Date(req.query.month+"-01").setMonth(date.getMonth()+1));
