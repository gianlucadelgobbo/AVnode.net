...
router.putData = async (req, res, view) => {
 ...
 /*  if (req.body.mediastr) {
  ...
  let data;
  try {
    data = await model.findById(id, config.cpanel[req.params.sez].forms[req.params.form].select).exec();
    if (!data) {
...
/* if (data.emails){
  ...
  try {
    await helpers.setStatsAndActivity(query);
  } catch (err) {
  ...
  try {
    const updatedData = await model.findById(id).select(select).populate(populate).exec();
    if (!updatedData) {
      if (view == "json") {
