...
import {arrayOfPerformance, performance} from '../../schemas/performance'
...
export const fetchList = () => (dispatch) => {
    ...
    return api.fetchPerformances()
        .then(
            (response) => {
                dispatch({
                    type: FETCH_LIST_SUCCESS,
                    response: normalize(response || [], arrayOfPerformance)
                });
            },
            (error) => {
                dispatch({
...
export const removeModel = ({id}) => (dispatch) => {
    ...
    return api.removePerformance({id})
        .then(
            (response) => {
                dispatch({
...
export const saveModel = (model) => (dispatch) => {
    ...
    return api.postPerformance(model)
        .then(
            (response) => {
                dispatch({
                    type: SAVE_MODEL_SUCCESS,
